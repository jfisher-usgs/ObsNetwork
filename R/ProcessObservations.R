ProcessObservations <- function(d, x.var, y.var, projargs, siteno.var,
                                sitenm.var, net.var, var1.var, var2.var,
                                acy.var, dt.var, dt.lim,
                                dt.fmt="%Y-%m-%d %H:%M") {

  # Reduce size of data frame
  d <- d[, c(x.var, y.var, siteno.var, sitenm.var, net.var, var1.var, var2.var,
             acy.var, dt.var)]

  # Rename variable names
  var.names <- c("x", "y", "siteno", "sitenm", "net", "var1", "var2",
                 "acy", "dt")
  names(d) <- var.names
  ...
  # Remove NA values
  is.valid.rec <- !(is.na(d$siteno) | is.na(d$var1) | is.na(d$var2) |
                    is.na(d$dt))
  d <- d[is.valid.rec, ]
  ...
  n <- nrow(obs)
  obs[, c("x", "y", "mapid", "var1", "var2", "acy", "sd", "net",
          "sitenm")] <- NA
  for (i in 1:n) {
    obs$mapid[i] <- i
